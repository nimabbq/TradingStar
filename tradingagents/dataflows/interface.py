...
from .akshare_fund import (
    get_fundamentals as get_akshare_fund_fundamentals,
    get_indicator as get_akshare_fund_indicator,
    get_news as get_akshare_fund_news,
...
VENDOR_METHODS = {
    ...
    # news_data
    "get_news": {
        "akshare_fund": get_akshare_fund_news,
...
def _should_use_china_fund_vendor_config(
    config: dict,
    category: str,
    method: str,
    args: tuple,
    ...
    if method in config.get("tool_vendors", {}):
        return False
    symbol = _extract_symbol(method, args, kwargs)
    if not symbol or not is_china_fund_symbol(symbol):
        return False
    configured = config.get("data_vendors", {}).get(category, "default")
    default = DEFAULT_CONFIG.get("data_vendors", {}).get(category, "default")
    return configured == default
